...
# ============================================================
# Load model bundle from GitHub Release
# ============================================================
...
def extract_models(model_bundle):
    models = {}

    if not isinstance(model_bundle, dict):
        return models

    if "selected_models" in model_bundle and isinstance(model_bundle["selected_models"], dict):
        for target, obj in model_bundle["selected_models"].items():
    ...
    elif "models" in model_bundle and isinstance(model_bundle["models"], dict):
        for target, estimator in model_bundle["models"].items():
            models[normalize_target_name(target)] = estimator

    elif "target_models" in model_bundle and isinstance(model_bundle["target_models"], dict):
        for target, estimator in model_bundle["target_models"].items():
            models[normalize_target_name(target)] = estimator
...
if not MODELS:
    st.error("لم يتم العثور على النماذج داخل model bundle.")
    if isinstance(bundle, dict):
        st.write("Available bundle keys:", list(bundle.keys()))
    st.stop()
...
def extract_global_features(model_bundle, meta, models):
    candidate_keys = [
        "features",
        "feature_columns",
        "input_features",
    ...
    if isinstance(model_bundle, dict):
        for key in candidate_keys:
            if key in model_bundle and model_bundle[key]:
                return list(model_bundle[key])
    ...
    for estimator in models.values():
        inferred = infer_features_from_model(estimator)
        if inferred:
            return inferred
...
FEATURES = extract_global_features(bundle, metadata, MODELS)
...
def extract_options(meta):
    ...
    if "options" in meta and isinstance(meta["options"], dict):
        opts.update(meta["options"])

    mapping = {
        "skin_type_options": "skin_type",
        "tattoo_color_options": "tattoo_color",
        "tattoo_type_options": "tattoo_type",
        "laser_type_options": "laser_type",
    }

    for source_key, target_key in mapping.items():
        if source_key in meta and meta[source_key]:
            opts[target_key] = meta[source_key]
...
# ============================================================
# Prediction value helper - IMPORTANT FIX
# ============================================================
...
def add_patient(full_name, file_number, age, gender, notes):
    ...
    cur.execute(
        """
        INSERT INTO patients(full_name, file_number, age, gender, notes, created_at)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (
            full_name,
...
def add_session(patient_id, session_number, row, preds):
    ...
    cur.execute(
        ...
        (
            ...
            float(get_wavelength(preds)),
            float(get_energy(preds)),
            float(get_pulse_duration(preds)),
            float(get_total_pulses(preds)),
            datetime.now().strftime("%Y-%m-%d %H:%M"),
        ),
    )
...
TEXT = {
    "English": {
        ...
        "all_sessions": "All Treatment Sessions",
        "full_name": "Full Name",
        ...
        "disclaimer": (
            "These AI-generated recommendations are intended to assist clinical decision-making "
            "and should not replace professional medical judgment. All treatment parameters must "
            "be verified by a qualified dermatologist or laser specialist before application."
        ),
        "about_text": (
            "This web-based prototype is connected to the trained machine-learning models developed "
            "in the thesis. It receives patient, tattoo, and device-aware inputs, then predicts "
            "Wavelength, Energy, Pulse Duration, and Total Pulses as guideline-support outputs. "
            "The machine-learning models run in the background; the clinical user sees only "
            "interpretable treatment recommendations."
        ),
    },
    "العربية": {
        ...
        "all_sessions": "جميع الجلسات العلاجية",
        ...
        "disclaimer": (
            "هذه التوصيات الناتجة من النظام الذكي مخصصة لدعم القرار السريري فقط، ولا تُعد بديلاً "
            "عن رأي الطبيب أو اختصاصي الليزر. يجب مراجعة جميع معاملات العلاج قبل التطبيق العملي."
        ),
        "about_text": (
            "هذا النموذج الأولي يعمل كتطبيق ويب مرتبط بنماذج التعلم الآلي التي تم تدريبها ضمن الرسالة. "
            "يستقبل النظام خصائص المريض والوشم والجهاز، ثم يتنبأ بالطول الموجي والطاقة ومدة النبضة "
            "وعدد النبضات كمخرجات إرشادية. تعمل النماذج في الخلفية ولا تظهر للمستخدم إلا التوصيات "
            "القابلة للفهم السريري."
        ),
    },
}
...
def clean_color_options(options):
    ...
    for item in options or []:
        for part in str(item).lower().replace(";", ",").split(","):
            p = part.strip()
...
def build_row(
    ...
    for feature in FEATURES:
        ...
        elif key in ["colors", "tattoo_color", "ink_color"]:
            row[key] = color

        elif key == "size_cm2":
            row[key] = float(size_cm2)

        elif key == "tattoo_age_years":
            row[key] = float(tattoo_age_years)

        elif key == "tattoo_type":
            row[key] = tattoo_type

        elif key in ["laser_type", "laser_type_sanitized"]:
            row[key] = laser_type

        elif key == "repetition_rate_hz":
            row[key] = float(repetition_rate_hz)

        else:
            row[key] = np.nan

    # Add standard aliases in case some pipelines accept extra columns.
    row.setdefault("skin_type", skin_type)
    row.setdefault("tattoo_color", color)
    row.setdefault("colors", color)
    row.setdefault("size_cm2", float(size_cm2))
    row.setdefault("tattoo_age_years", float(tattoo_age_years))
    row.setdefault("tattoo_type", tattoo_type)
    row.setdefault("laser_type", laser_type)
    row.setdefault("laser_type_sanitized", laser_type)
    row.setdefault("repetition_rate_hz", float(repetition_rate_hz))

    return row


def predict_single_model(model, row):
    model_features = infer_features_from_model(model)
...
def predict_outputs(row):
    ...
    for target, model in MODELS.items():
        try:
            predictions[target] = predict_single_model(model, row)
        except Exception as e:
            predictions[target] = np.nan
            predictions[f"{target}__error"] = str(e)
...
def export_prediction_report(patient, row, preds, T):
    ...
    if patient:
        rows.append(["Patient", patient.get("full_name", "")])
        rows.append(["File Number", patient.get("file_number", "")])

    for k, v in row.items():
        rows.append([k, v])

    rows.append(["Predicted Wavelength_nm", get_wavelength(preds)])
    rows.append(["Predicted Energy_Jcm2", get_energy(preds)])
    rows.append(["Predicted Pulse Duration", get_pulse_duration(preds)])
    rows.append(["Predicted Total Pulses", get_total_pulses(preds)])
...
with st.sidebar:
    st.markdown(
        ...
        <div class="sidebar-logo">
            ...
            <div class="logo-text">Laser DSS</div>
        </div>
        """,
        unsafe_allow_html=True,
    )

    T_side = TEXT.get(st.session_state.language, TEXT["English"])
    ...
    st.markdown(
        ...
        <div class="user-box">
            ...
            <div>
                <b>Sury Hop</b><br>
                <span class="muted">suryhop@gmail.com</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
if st.session_state.view == "dashboard":
    ...
    with c2:
        st.markdown(
            ...
            <div class="metric-card">
                ...
                <div class="metric-value">{len(sessions_df)}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        f'<div class="card"><div class="section-title">🕘 {T["recent_sessions"]}</div>',
        unsafe_allow_html=True,
    )
    ...
    if sessions_df.empty:
        ...
        for _, s in sessions_df.head(5).iterrows():
            st.write(
                f"**Session {int(s.session_number)} — {s.colors}**  \n"
                f"Patient: {s.full_name} · Wavelength: {round(s.wavelength)} nm · Energy: {round(s.energy, 3)}"
            )
    ...
    with st.expander("➕ " + T["add_patient"], expanded=False):
        st.markdown(
            f'<div class="card"><div class="section-title">{T["patient_information"]}</div>',
            unsafe_allow_html=True,
        )
    ...
    if search.strip() and not view_df.empty:
        q = search.lower()
        view_df = view_df[
            view_df.full_name.astype(str).str.lower().str.contains(q, na=False)
        ...
        for _, p in view_df.iterrows():
            ...
            st.markdown(
                ...
                <div class="patient-card">
                    ...
                    <b>{p.full_name}</b><br>
                    <span class="muted">File: {p.file_number}</span><br>
                    <span class="muted">Age: {p.age}</span><br>
    ...
    if not patients_df.empty:
        st.download_button(
            ...
            patients_df.to_csv(index=False).encode("utf-8-sig"),
    ...
    st.markdown(
        f'<div class="card"><div class="section-title">📋 {T["all_sessions"]}</div>',
        unsafe_allow_html=True,
    )
    ...
    if not p:
        ...
        with a:
            st.write("**" + T["full_name"] + "**")
            st.write(p["full_name"])
            st.write("**" + T["age"] + "**")
            st.write(p["age"])

        with b:
            st.write("**" + T["file_number"] + "**")
            st.write(p["file_number"])
            st.write("**" + T["gender"] + "**")
            st.write(p["gender"])
        ...
        st.markdown(
            f'<div class="card"><div class="section-title">〰️ {T["treatment_sessions"]}</div>',
            unsafe_allow_html=True,
        )
        ...
        if pdf.empty:
            ...
            for _, s in pdf.iterrows():
                st.write(
                    f"**Session {int(s.session_number)} — {s.colors}**  \n"
                    f"Wavelength: {round(s.wavelength)} nm · Energy: {round(s.energy, 3)} · "
                    f"Pulse Duration: {round(s.pulse_duration, 4)} ns · Total Pulses: {round(s.total_pulses)}  \n"
                    f"{s.created_at}"
                )
            ...
            st.download_button(
                ...
                f"patient_{p['file_number']}_sessions.csv",
                "text/csv",
                use_container_width=True,
            )
    ...
    if selected_patient:
        st.info(
            f"{T['new_prediction']} for: {selected_patient['full_name']} | "
            f"File: {selected_patient['file_number']}"
        )

    skin_options = safe_options(
        OPTIONS.get("skin_type", []),
        ["II", "III", "IV", "V", "VI"],
    )

    color_options = clean_color_options(
        OPTIONS.get("colors", OPTIONS.get("tattoo_color", []))
    )

    tattoo_type_options = safe_options(
        OPTIONS.get("tattoo_type", []),
        ["simple", "complex", "amateur", "professional"],
    )

    laser_type_options = safe_options(
        OPTIONS.get("laser_type", OPTIONS.get("laser_type_sanitized", [])),
        ["Q-switched Nd:YAG", "Picosecond", "Alexandrite", "Ruby"],
    )
    ...
    with left:
        st.markdown(
            f'<div class="card"><div class="section-title">{T["patient_profile"]}</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            f'<div class="age-pill">{tattoo_age_years:.2f} years</div>',
            unsafe_allow_html=True,
        )
    ...
    with right:
        st.markdown(
            f'<div class="card"><div class="section-title">{T["tattoo_characteristics"]}</div>',
            unsafe_allow_html=True,
        )

        size_cm2 = st.number_input(
            T["tattoo_size"],
            min_value=0.1,
            max_value=1000.0,
            value=15.0,
    ...
    st.markdown(
        f'<div class="card"><div class="section-title">{T["device_inputs"]}</div>',
        unsafe_allow_html=True,
    )
    ...
    with d2:
        repetition_rate_hz = st.number_input(
            T["repetition_rate"],
            0.0,
            50.0,
            5.0,
    ...
    if st.session_state.last_prediction is not None:
        ...
        wavelength_value = get_wavelength(preds)
        energy_value = get_energy(preds)
        pulse_duration_value = get_pulse_duration(preds)
        total_pulses_value = get_total_pulses(preds)
        ...
        st.markdown(
            f'<div class="card"><div class="section-title">{T["recommended"]}</div>',
            unsafe_allow_html=True,
        )
        ...
        with r1:
            show_result_card("〰️", T["wavelength"], w, "nm")

        with r2:
            show_result_card("⚡", T["energy"], e, "J/cm²")

        r3, r4 = st.columns(2)

        with r3:
            show_result_card("⏱️", T["pulse_duration"], pdur, "ns")

        with r4:
            show_result_card("#", T["total_pulses"], tp, "pulses")
        ...
        st.markdown(
            ...
            <div class="disclaimer">
                ⚠️ <b>{T["clinical_disclaimer"]}:</b> {T["disclaimer"]}
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.download_button(
            "📄 " + T["download_report"],
            report,
            "laser_tattoo_prediction_report.xlsx",
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )

        st.markdown(
            f'<div class="card"><div class="section-title">💾 {T["save_to_record"]}</div>',
            unsafe_allow_html=True,
        )
        ...
        if patients_df.empty:
            ...
            labels = [
                f"{x.full_name} | File: {x.file_number}"
                for _, x in patients_df.iterrows()
            ]
    ...
    st.markdown(
        f'<div class="card"><div class="section-title">{T["about"]}</div>',
        unsafe_allow_html=True,
    )
